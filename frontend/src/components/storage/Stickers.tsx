        ...
        <div className="w-full grid grid-cols-3 space-y-2 gap-2 mt-6">
            {data.map((pack: any) =>
                <div key={pack.id} className="flex flex-col items-center">
                    ...
                    <p className="text-primary-text/80 text-sm font-extralight flex flex-row gap-2 items-center bg-third-background px-2 py-1 rounded-2xl">{pack.floor_price.toFixed(2)}<img alt='TON' src='https://ton.org/download/ton_symbol.png' className="object-cover w-4 h-4 rounded-full" /></p>
                </div>
            )}
        </div>
    )
